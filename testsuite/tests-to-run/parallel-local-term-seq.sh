...
echo '#####################'
nproc=5
parallel --timeout 2 --term-seq TERM,5,TERM,5,INT -k -j $nproc $measure_term_seq ::: {0..99} &
parallel_pid=$!
...
echo '#############################################'
parallel --propagate-term-seq -k -j $nproc $level1_prog ::: {0..99} &
parallel_pid=$!
echo "Sending kill -TERM in 5 seconds..."; sleep 5; kill -TERM $parallel_pid
