...
=item B<--propagate-sigterm1, --propagate-sigterm2>

The first time GNU B<parallel> receives the B<SIGTERM> signal, it
stops dispatching new jobs, and waits for running jobs to complete.

If you wish for GNU B<parallel> to propagate the first B<SIGTERM> signal to
the running jobs, use the B<--propagate-sigterm1> option.

The second time GNU B<parallel> receives the B<SIGTERM> signal, it
exits with no further actions. The running jobs are abandonned to the OS.

If you wish for GNU B<parallel> to propage the second B<SIGTERM> signal to
the running jobs, use the B<--propagate-sigterm2> option.
...
If you send B<SIGTERM> a second time, GNU B<parallel> will quit
without waiting for the jobs to be finished.

=head1 TERMINATE RUNNING JOBS AND DO NOT START NEW JOBS

If you wish for GNU B<parallel> to propagate the B<SIGTERM> signal to
the jobs that are still running, you have two choices.

You can tell GNU B<parallel> to propagate the TERM signal the first
time you sent the B<SIGTERM> signal:

  parallel --propagate-sigterm1 sleep 120 ::: 1 & pid=$! 
  kill -TERM $pid

or the second time you sent the B<SIGTERM> signal.

  parallel --propagate-sigterm2 sleep 120 & pid=$!
  kill -TERM $pid
  kill -TERM $pid

GNU B<parallel> gives your program a grace period of to 200 milliseconds
to cleanup before it itself exits. If for some reason you want to extend
this grace period, you can use the B<--wait-for-child GRACE_PERIOD>
option with a B<GRACE_PERIOD> grace period specified in seconds.
